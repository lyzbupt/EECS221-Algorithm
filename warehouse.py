...
class warehouse():
	...
	def runGUI(self):
		...
		def process():
			...
			if self.mode == 'A' :
				if self.algorithm == 'A':   # near neighbor
					...
					drawMap(OptimalItem)
					logStr = "Near neighbor process. Optimal order is "+str(OptimalItem)+" Cost: "+str(optCost)
					txt.insert(END, logStr)
				else:						# branch bound
					cost, order = bb.branchBound(self.beginning, self.destination, self.itemsID,self.considerWeight)
					drawMap(order)
					logStr = "Branch bound process. Optimal order is "+str(order)+" Cost: "+str(cost)
					txt.insert(END, logStr)
			elif self.mode == 'B':              # batch processing
					...
					with open( fileName, 'rb') as f:
						...
						for line in lines:
							...
							if(i == orderNumber):
								...
								self.beginning = eval(c[0])
								self.destination = eval(c[1])
								self.itemsID = eval(c[2])
								nnOrder = eval(c[3])
								nnCost = c[4]
								bbOrder = eval(c[5])
								bbCost = c[6]
								#print c[3],eval(c[3]), type(c[3]), type(eval(c[3])),type(c[5]),type(c[6])
								logStr = "For order "+str (i)+ ". The start location is: "+str( self.beginning)+ " End location is: "+str(self.destination)+"\n"
								txt.insert(END, logStr)
								logStr = "Original order: "+str(self.itemsID)+ ". nn Optimal order: "+str(nnOrder)+ " Cost: "+str(nnCost)+"\n"
								txt.insert(END, logStr)
								logStr = "Original order: "+str(self.itemsID)+ ". bb Optimal order: "+str(bbOrder)+ " Cost: "+str(bbCost)+"\n"
								txt.insert(END, logStr)
								if self.algorithm == 'A':
									drawMap(nnOrder)
								else:
		...
		def drawMap(order):
			...
			for pa in path:
				for p in pa:
					Label(rightFrame, text="    ", bg='yellow').grid(row =p[1]-1,column =p[0]-1)
			Label(rightFrame, text="    ", bg='red').grid(row =self.beginning[1]-1,column =self.beginning[0]-1)
			Label(rightFrame, text="    ", bg='red').grid(row =self.destination[1]-1,column =self.destination[0]-1)
			for item in order:
				p = grid[item]
				Label(rightFrame, text="    ", bg='blue').grid(row =p[1]-1,column =p[0]-1)
